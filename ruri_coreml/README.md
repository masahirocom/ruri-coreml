...
```bash
# 1. Convert a checkpoint to Core ML (fp32 recommended — see conversion.py's
#    docstring for a real-device fp16 numerical issue this avoids).
python -m ruri_coreml_convert convert cl-nagoya/ruri-v3-130m ruri-v3-130m 128 --precision fp32

# 2. Optionally shrink it with int8 weight quantization.
python -m ruri_coreml_convert quantize ruri-v3-130m_seq128_fp32.mlpackage ruri-v3-130m_seq128_int8.mlpackage cl-nagoya/ruri-v3-130m 128
...
- **fp16 models can return NaN on real iPhones (not the Simulator)** — see `conversion.py`'s module docstring. Prefer `--precision fp32` for anything shipping to a real device.
